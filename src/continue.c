...
// continues evolution started by combie.c from the last saved point. only new features are commented, for the rest see combie.c
...
int run(void)
{
    ...
    float rough_dt = 1E-2;
    int rough_steps = 101;
    int num_tsteps = (rough_steps-1)*round(rough_dt/dt)+1;
    double t;
    int ind;
    double A_a;
    double A_v;
    ...
    // SET LAST SAVED DATA POINT NUMBER HERE (currently it's '501')
    itoa(501,filename,10); // load last saved placenta volume vector
    ...
    step = 0;
    t = 5+dt*step; // add time at last saved point (at the moment, it's '5')

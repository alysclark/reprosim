...
!subroutine bsten ( nx, ny, nz, kx, ky, kz, nfree, stencil, h )		! REMOVED
...
!
  ...
  real ( kind = 8 ) eps
  real ( kind = 8 ) errst
  real ( kind = 8 ) hh(ih0,ih0)		! Gib added dimensions
  integer ( kind = 4 ) ierr
  integer ( kind = 4 ) ih
  integer ( kind = 4 ) indic
  integer ( kind = 4 ) job
...
!
!    call oped ( n, x, y, a, ioff, ndiag )		! REMOVED
  ...
  do iz = 1, nz
    do iy = 1, ny
      do ix = 1, nx
        ...
        ia(node) = iedge
!        call bsten ( nx, ny, nz, ix, iy, iz, nfree, stencil, h )	! REMOVED
!
!  West
!
        if ( 1 < ix ) then
  ...
  do iz = 1, nz
     do iy = 1, ny
        do ix = 1, nx
           ...
           ia(node) = iedge
!           call getsten ( nx, ny, nz, ix, iy, iz, stencil, h )		! REMOVED
!
!  West
!
           if ( 1 < ix ) then
